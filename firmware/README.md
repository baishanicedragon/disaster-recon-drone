...
2. **无动力改平段控制器（段 II：CUTOFF → DEPLY → FLARE）**——关机后俯冲增速、
   门控展翼、2.0 g 拉平的时序状态机（见 [docs/08](../docs/08-自主导航与山区穿越.md) 8.6）。
...
│   ├── src/deploy_ctrl.c         ← 段 II：关机-俯冲-展翼门控-2.0g 拉平时序
...
- [ ] 关机 → 俯冲 → 展翼门控（空速 + 高度硬门控逻辑）→ 2.0 g 拉平 → 平飞
- [ ] 地形跟随（加载本地 DEM）
...
| 层 | 工具 | 范围 |
|:---|:---|:---|
| 单元 | 简易 C test / Unity | 倒拖阻尼增益、展翼门控、改平时序、能量管理判据 |
